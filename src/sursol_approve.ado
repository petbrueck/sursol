...
noi dis as error _n "Attention. There is no protocol specified in {help sursol_approve##server:server({it:url})}"
...
noi dis as result "0 interviews will be approved by Supervisor."
}
...
`"	     "'  _newline ///			 																										  ///
 `"for (x in c("user", "password", "server")) {  "'    _newline ///
                ...
                `"    print(paste("Check that the parameters in the data are the correct data type (e.g. String?). Look at:",x))  "'    _newline ///
                ...
                `"    print(paste("The following parameter is not specified in the program:", x))  "'    _newline  ///
                `"    Sys.sleep(5)  "'    _newline ///
                `"    stop()  "'    _newline ///
                `"  }  "'    _newline ///
                `"}  "'    _newline ///
`"command <- "/approve?comment=`comment'""' _newline ///
`"counter=0"' _newline ///
`"count406=0"' _newline ///
`"count404=0"' _newline ///
...
`" 	}  "' _newline ///
 ...
 `"print(paste("Target interview", val," was in status that was not ready to be approved")) "' _newline ///  
...
`"print(paste(count200,"interviews have been successfully approved by Supervisor")) "' _newline ///  
`"print(paste(count406,"interviews have been in status that was not ready to be approved by Supervisor")) "' _newline ///  
...
tempfile error_message //ERROR MESSAGES FROM R WILL BE STORED HERE
		timer clear
		timer on 1
shell "`rpath'/R" --vanilla <"`c(pwd)'/approve.R" 2>`error_message' 
		timer off 1	
		qui timer list 1
		if `r(t1)'<=3 {
		noi dis as error "Whoopsa! That was surprisingly fast."
		noi dis as error "Please check if the data was downloaded correctly." 
		noi dis as error "If not, have a look at {help sursol_approve##debugging:debugging information} in the help file."
