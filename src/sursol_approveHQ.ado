...
`"approve <- PATCH(approve_query, authenticate(user, password))"' _newline ///
 ...
 `"message(paste("Target interview", val," was in status that was not ready to be approved")) "' _newline ///  
...
`"message(paste(count200,"interviews have been successfully approved by HQ")) "' _newline ///  
`"message(paste(count406,"interviews have been in status that was not ready to be approved by HQ")) "' _newline ///  
