...
noi dis as error _n "Attention. There is no protocol specified in {help sursol_approveHQ##server:server({it:url})}"
...
`"	     "'  _newline ///			 																										  ///
 `"for (x in c("user", "password", "server")) {  "'    _newline ///
                ...
                `"  }  "'    _newline ///
                `"}  "'    _newline ///
`"command <- "/hqapprove?comment=`comment'""' _newline ///
`"counter=0"' _newline ///
`"count406=0"' _newline ///
`"count404=0"' _newline ///
...
`"print(paste(count200,"interviews have been successfully approved by HQ")) "' _newline ///  
`"print(paste(count406,"interviews have been in status that was not ready to be approved by HQ")) "' _newline ///  
...
shell "`rpath'/R" --vanilla <"`c(pwd)'/approve.R" 2>`error_message' 
		...
		noi dis as error "Please check if the have been approved by HQ." 
		noi dis as error "If not, have a look at {help sursol_approveHQ##debugging:debugging information} in the help file."
