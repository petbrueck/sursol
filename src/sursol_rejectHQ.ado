...
noi dis as error _n "Attention. There is no protocol specified in {help sursol_rejectHQ##server:server({it:url})}"
...
noi dis as error  "Please specify the path of your R.exe using the option {help sursol_rejectHQ##sursol_rejectHQ_rpath:rpath(string)}"
ex
}

if length("`comment'")==0 loc comment ""
...
ex
	...
	noi dis as error "`x'" 
	if `length'==`i' {
	noi dis as result _n "Version `x' of R will be used to reject the interviews"
	loc version="`x'"
	} 
	loc ++i
	} 
...
noi dis as error  "Please correctly specify the path of your R.exe using the option {help sursol_rejectHQ##sursol_rejectHQ_rpath:rpath(string)}"
ex
}
}
...
`"	}     "'  _newline	 ///																																  ///
 `"for (x in c("user", "password", "server")) {  "'    _newline ///
                ...
                `"  }  "'    _newline ///
                `"}  "'    _newline ///
`"command <- "/hqreject?comment=`comment'""' _newline ///
`"counter=0"' _newline ///
`"count406=0"' _newline ///
`"count404=0"' _newline ///
...
`"print(paste(count200,"interviews have been successfully rejected by HQ")) "' _newline ///  
