...
noi dis as error  "Please specify the path of your R.exe using the option {help ss_unapproveHQ##ss_unapproveHQ_rpath:rpath(string)}"
ex
}
...
noi dis as error  "Please correctly specify the path of your R.exe using the option {help ss_unapproveHQ##ss_unapproveHQ_rpath:rpath(string)}"
ex
}
}
...
`"unapprove <- PATCH(unapprove_query, authenticate(user, password))"' _newline ///
 ...
 `"message(paste("Target interview", val," was in status that was not ready to be rejected by HQ")) "' _newline ///  
...
`"message(paste(count200,"interviews have been successfully unapproved by HQ")) "' _newline ///  
`"message(paste(count406,"interviews have been in status that was not ready to be rejected by HQ")) "' _newline ///  
`"message(paste(count404,"interviews have been not found")) "' _newline ///  
`"  Sys.sleep(5) "' _newline ///
`"}"' _newline ///
`"}"'	_newline
 file close rcode 
shell "`rpath'\R" --vanilla <"`c(pwd)'\unapprove.R"
qui capt rm "`c(pwd)'\unapprove.R"
qui capt rm "`c(pwd)'\.Rhistory" 
}
}
