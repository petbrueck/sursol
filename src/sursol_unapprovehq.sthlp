...
The command prompt/terminal returns warnings if (1) interview was in status that was not ready to be (un)approved or (2) interview has not been found. {p_end}

{pstd}
Interviews can be approved as Headquarter with the following status only: {it:Completed} and {it:Approved by Supervisor}.  {p_end}
...
{phang}
{opt rpath(string)} specifies the path to the R application through which the data export request to the server is transmitted. {opt sursol unapproveHQ} assumes that the executable can 
...
{pstd}Load survey dataset and unapprove interviews if they had errors flagged by the Survey Solutions software{p_end}

{phang2}{cmd:. use "C:\Users\survey_X.dta" ,clear }{p_end}

{phang2}{cmd:. sursol unapproveHQ if has__errors>0, server("https://projectX.mysurvey.solutions") /// }{p_end}
{phang2}{cmd:. user("API_acc") ///  }{p_end}
{phang2}{cmd:. password("API_acc2019")  /// }{p_end}
{phang2}{cmd:. comment("Unapproved by API User as there was an error which has not been considered before.") }{p_end}
...
{pstd}{cmd:sursol [un]approveHQ} was last updated using Survey Solutions 20.10
